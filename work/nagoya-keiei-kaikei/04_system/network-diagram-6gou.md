...
- **★.7.241クライアントモニター（2026-06-24）＝2.4G(11g)接続0台・WDS 0台**＝**現状ほぼ無人**（techinfoのVLAN7 IPv4リース0と整合）。5GHz(11a)モニターも空なら「クライアント0で確定」。SSID名`NAMATESC01`が"test"風＝廃止/テスト残りの可能性も。→ 解釈2択（①実質未使用 ②時間帯/学期で一時的に無人）、断定せず。**いずれにせよ生徒無線VLAN7の実利用は薄い＝N-02では生徒APを必要箇所に絞る（過剰投資回避）の判断材料**。
