...
- ⚠ **物理設置リスク（2026-06-24 実機）＝機器が無固定**：2F給湯室の配電盤内に **ONU・RTX1210・FortiGate・SW HUB が固定されず詰め込み**。**パネルを開けると機器が落ちてくる**状態。→ **落下＝ケーブル/コネクタ張力→抜け・破損→6号館全館ネット断**のリスク（基幹がこの1箱に集約）。保守のたびに事故リスク・放熱悪（RTX44℃）・整線なし。**「設計された設備」でなく「積み上がった結果」**。
  - **N-02スコープに「適正収納・固定・整線・放熱」を明記**（ラックマウント/固定金具＋結束・ラベリング）＝総合管理の価値。
  - **暫定策（N-02前）**：落下で全館断の恐れ＝**当面の固定だけ先行提案**も可。ただし**配電盤内＝電気工事区分の確認必須**（勝手にいじらない）。
...
> **現地計測方針（2026-06-24・確定）**：階高は**精密全階計測はしない**。**2Fのみ代表1点を計測**し、配線距離は「2F階高×階数」で概算。各階は**「天井は脚立で届くか／吹き抜けの有無」**の◯×のみ記録（高所作業＝足場/昇降機の要否が工賃の変数）。生徒無線が実質未使用のため**無線カバレッジ設計向けの天井高精査は不要**。
>
> **2F階高 実測＝2.709m（2026-06-24）**。riser縦配線の概算根拠＝2.709m×フロア数（2F〜8F＝約6層分）。※この値が天井高ベースなら床〜床はスラブ厚分を上乗せ（概算+0.3〜0.5m）。**配線長見積はこの代表値で全階一律換算**（精密計測省略の方針通り）。
...
| 2F机上AP | .2.200 | **WAPM-1166D（2F机上・むき出し・FG迂回・シャドーAP）**。FW1.12.1。**SSID=`NGOKAJP36`(生徒)を VLAN Untagged/1 で配信＝生徒SSIDを事務VLAN1に橋渡し＝VLAN分離の抜け穴**。暗号はWPA2-AES（3F-1はTKIP混在＝設定不統一）。エアステーション名=既定（MAC由来・命名未設定）・2.4G出力10%＝**現場の後付け増設**。旧資料「3F生徒.200」は場所誤記＝同一1台 |
...
| ★カメラ | .2.x **多数（TP-Link製）** | **Tapo C110 が大量にVLAN1（事務系）に同居** |
| シャドー機器 | **.2.7＝BUFFALO WEX-733DHP（Wi-Fi中継器・民生／FW1.61）** | ★スキャンで"Redirect Page/MAC=0"だった正体＝FGではなく中継器。VLAN1上の**後付け民生機**。台帳PW（→credentials）では入れず＝別系統。撤去/正規化対象 |
...
## VLAN7スキャン実機（2026-06-24・PCをVLAN7側ポートに接続）★分離の良い実例を裏付け

VLAN7(192.168.7.0/24)を作業PCから直接スキャン。**3台のみ＝極めてクリーン**。

| 区分 | IP（VLAN7） | 実機 |
|---|---|---|
| GW | .7.254 | RTX1210（VLAN7のGW＝secure filterで事務/他VPN網と遮断） |
| 生徒AP | **.7.241** | **BUFFALO WAPM-1166D（＝2F机上シャドー.200と同一型番）**。.200の兄弟機（MAC/ログインPW→credentials・.200と同一弱PW）。SSID=**NAMATESC01**。**FW 1.4.3**（★.200は1.12.1＝同型でFW不統一）・命名既定（MAC由来＝未設定）・有線#1=100M全二重Untagged/1。**★NTP無効・内蔵時計2013/9/1（.200シャドーと同じ約13年ズレ）** |
| 作業PC | .7.26 | スキャン中のYutaka PC（SOLUTIONSYS/Lenovo）＝DHCP scope2が正常採番（techinfoの「リース0」は無人だっただけと判明） |

- **VLAN7は生徒AP＋GWだけ＝事務VLAN1(104台密集)と対照的に素直に分離**。**6号館＝校内で唯一の“分離済み良い実例”をさらに裏付け**。N-02で他棟を揃える社内モデル。
- 留意：.7.241も**AP管理PWが.200シャドー機と同一の推測しやすい弱PW**（実値→credentials）＝**全AP共通の弱PW運用**の疑い。N-02で全機PW刷新の根拠。

### ★.7.241 VLAN設定画面（2026-06-24）＝ポートベースVLANの動作を実証
- **.7.241のAP内部VLAN設定は全てUntagged/VLAN ID=1**（有線#1/#2・SSID NAMATESC01・管理VLAN全て）。**APはVLANを喋らない（出荷既定）**。にもかかわらずIPは192.168.7.241＝VLAN7。
- → **6号館はタグVLAN(802.1Q)ではなくポートベースVLAN**で確定。**セグメントを決めるのはAP設定ではなく“ケーブルが挿さるRTXポート”**。このAPはVLAN7ポートに挿さっているのでRTX側でVLAN7へ強制収容され、正しく隔離されている。
- ★**決定的な対比＝シャドーAP問題の本質**：**.7.241 と 2F机上シャドー.200 は同一型番(WAPM-1166D)・同一既定設定(Untagged/VLAN1)**。違いは挿し先ポートだけ — .241=VLAN7ポート(✅隔離)／.200=VLAN1ポート(❌生徒SSIDが事務VLAN1に漏れる)。→ **シャドーAPはAP設定不良ではなく“ケーブルが事務ポート”なだけ＝挿し替えで即是正＝低コストで分離の穴を塞げる**（N-02の強力実例）。
- ★**ポートベースVLANの限界**：APがVLAN非対応＝**1本のケーブルに教員+生徒2SSIDを混載不可＝SSIDごとに物理ケーブル必須**。これが「1号館品質（管理SWのタグトランク）へ巻き直す余地」の技術的根拠。
- **★.7.241クライアントモニター（2026-06-24）＝2.4G(11g)・5G(11a)・WDS すべて0台で確定**＝**現状クライアント皆無**（techinfoのVLAN7 IPv4リース0と整合）。SSID名`NAMATESC01`が"test"風＝廃止/テスト残りの可能性も。→ 解釈2択（①実質未使用 ②時間帯/学期で一時的に無人）、断定せず。**いずれにせよ生徒無線VLAN7の実利用は薄い＝N-02では生徒APを必要箇所に絞る（過剰投資回避）の判断材料**。
- 次：.7.241のシステム画面（FW版・NTP・暗号方式）取得→firmware-inventory反映。

## VLAN8スキャン実機（2026-06-24・port8接続）★枠だけ・AP不在で確定

VLAN8(192.168.8.0/24)を作業PCからport8経由でスキャン。**GW＋作業PCの2台のみ＝物理AP不在**。

| 区分 | IP（VLAN8） | 実機 |
|---|---|---|
| GW | .8.254 | RTX1210（VLAN8のGW） |
| 作業PC | .8.2 | スキャン中のYutaka PC（DHCP scope3が正常採番） |

- **VLAN8は枠（config port7-8・scope3・secure filter）はあるが物理APが1台も無い**。**スキャンでも現地の配線/設置確認でも8FらしいAPは未発見**（Yutaka目視）＝**8F生徒AP＝撤去 or 未設置で確定**。
- → 旧資料ベースの図中ノード「8F生徒AP（WAPM-1166D / 192.168.8.x）」は**実機で否定**。
- **★生徒網の総括**：VLAN7=AP有だが2.4/5G/WDS全0台（無人）／VLAN8=AP不在。**6号館の生徒無線は事実上稼働していない**。職員/7F/8Fの3VLAN分離設計は律儀だが、**生徒側は使われていない＝施工想定と実運用の乖離**。
- **N-02示唆**：分離設計は良いが**生徒無線の実需要が薄い→各階フル装備は不要・実利用箇所に絞り過剰投資回避**。ただし**「8Fは無線不要か／撤去か／今後使うか」を学校に要確認**（需要をゼロ断定しない）。
...
4. ~~**教員VLAN1 / 生徒VLAN7・8の分離**：物理かタグか~~ ✅**完全確定（6/24）**：RTXポートベース分割（lan1.1-4=vlan1 / 1.5-6=vlan7 / 1.7-8=vlan8）＋secure filter遮断。**5Fコア BS-GS2016P(.2.203)はVLAN設定なし＝VLAN1のみ全Untagged**＝VLAN1専用スイッチ。→ **VLAN分離は「RTXポート分割＋VLANごと別物理ケーブル」だけで成立、タグVLANトランクは皆無**。
   ...
   - ✅**RTX本体に施工者の物理ラベルあり（6/24実機）**：「**職員：LAN1-4／7F：LAN5-6／8F：LAN7-8**」。configのポートベースVLAN割当（lan1.1-4=vlan1／.5-6=vlan7／.7-8=vlan8）と完全一致＝**VLAN7=7F・VLAN8=8F がフロア対応で確定**。**日本語ラベルで明記＝ポートベースVLANは意図的な設計**（行き当たりばったりではない）。
