...
> 各階に給湯室配電盤(riser中継)がある構成。3Fは「下→3F→4F」、**4Fも同一パターン（給湯室配電盤・AP廊下天井・空きPF管あり／ラベル「5T-1へ」=5Fへ通過・配電盤に小JB分岐箱）**を確認＝**各階を経由する縦配管**。青/橙(FG迂回系)/緑が縦走。**空き管が各階にある＝1Fコアからのhome-run新設/光化が物理的に可能**（N-02工事の前提◎）。

---

## 設計の要点（写真から読めた現状の姿）※以下は4F以上＝写真ベース仮説
...
8. ~~**ネットワークカメラ**：資料に無し→有無~~ ✅**確定：各教室に設置あり（3Fで確認）**。残り＝接続先SW/VLAN・NVR(録画機)所在・IP・全館の台数を追跡。
