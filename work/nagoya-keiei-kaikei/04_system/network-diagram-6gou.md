...
| 生徒AP | **.7.241** | **BUFFALO AP（Mongoose httpd・login `admin/Sbm3241`）**。OUI 88:57:EE＝2F机上シャドー.200(C9:3E:40)の兄弟機。MAC→credentials。FW/型番/NTPはシステム画面で要取得 |
...
- 次：.7.241のシステム画面（FW版・NTP・SSID/暗号）取得→firmware-inventory反映。VLAN8(8F)も同様にport8から要スキャン。
