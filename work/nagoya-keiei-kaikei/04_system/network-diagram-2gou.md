...
RTX830 configを見ると決定的：
- **RTXは lan1=192.168.21.1/24 の単一フラットセグメントのみ（VLANなし・FG-LAN向けの静的ルートも無し）**。
- つまりRTXの認識では「192.168.21.0/24 は全部 lan1 直結（同一L2）」。.21.201 も同一セグメント扱い。
- それでも RTX直結PCから .21.201 に届かない＝**FortiGateが間でL2透過していない**。透過ブリッジなら同一セグメントとして当然届くはず。
→ **結論：FortiGate 50E は透過ではなく NAT/ルートモードで境界化しており、フロア側（FG-LAN）はRTXから到達不能な別領域になっている。**
　旧資料のHUB「192.168.21.201」は、FG透過時代 or FG導入前の値で、**現在のFG-LAN側の実アドレスは別セグメントの可能性が高い**（要・FG-LAN側ipconfig）。
→ 未使用館でフロア側にDHCPが来ないのは、FGのLAN側DHCPが無効/未設定、または末端未接続のため。
