...
| .21.4 | **TP-Link機(60:83:E7 OUI)＝Tapo C110カメラ**（全棟カメラ=Tapo C110と判明）。Wi-Fiでフラット.21網に同居 | ※当初「Apple AirMac疑い」は訂正 |
...
- **建物規模**：**2F〜7F まで各フロア2教室**（教室を挟んだ通路に EPS＝縦系の配管シャフト）。
  - ⚠ **要確認の差分**：フロア高計測 [2gou-floor-heights.md](2gou-floor-heights.md) は **1F〜4Fのみ**。実機APスキャンも **2F/3F（201/202・301/302）止まり**。**4F〜7Fの教室AP・配線・稼働は未確認**＝6/23時点で上層階は調査が届いていない。N-02見積前に上層階の現況（稼働教室数・AP有無）を要確定。
...
- ⚠ **「各フロア2教室」の整理**：教室フロアは 2F/3F で確認。**4Fは住居型研修（非教室・非配線）**＝用途が混在。先述の「2F〜7F各2教室」は要再確認（上層階＝教室か宿泊か未確定）。N-02の対象範囲（どの階を業務NW化するか）を学校と要すり合わせ。
